def setup_database():
	...
	'''NOT NULL contraint not listed under email_address for the case the user only wished to
	 provide one method of verification.'''
	create_sql = 'CREATE TABLE IF NOT EXISTS credentials (credential_id INTEGER PRIMARY KEY, username TEXT UNIQUE NOT NULL, email_address TEXT UNIQUE, password TEXT UNIQUE NOT NULL, salt TEXT UNIQUE NOT NULL, FOREIGN KEY(username) REFERENCES users(username))'
	create_table(create_sql)
	...
	'''NOT NULL constraints not listed under start time and location, price, or is_transit for
	 ...
	 friends.'''
	create_sql = 'CREATE TABLE IF NOT EXISTS trips (trip_id INTEGER PRIMARY KEY, username TEXT NOT NULL, trip_start_time DATETIME, trip_start_lat DECIMAL, trip_start_long DECIMAL, trip_stop_lat DECIMAL NOT NULL, trip_stop_long DECIMAL NOT NULL, price DECIMAL, is_transit BOOLEAN, FOREIGN KEY(username) REFERENCES users(username))'
	create_table(create_sql)
